...
import copy
from falcon.utils.zero_to_fp32 import convert_zero_checkpoint_to_fp32_state_dict
...
def evaluate_policy(
    ...
    if diverse_inst:
        with open("configs/data/calvin/lang_annotation_diverse.json", "r") as f:
...
def rollout(
    ...
    """Run the actual rollout on one subtask (which is one natural language instruction) with single action step"""
    if debug:
        print(f"{subtask} ", end="")
        time.sleep(0.5)
        img_list = []
    ...
    for _ in range(EP_LEN):
        action = model.step(obs, lang_annotation, env)
...
def rollout_act_chunk(
    ...
    """Run the actual rollout on one subtask (which is one natural language instruction) with one action chunk."""
    if debug:
        print(f"{subtask} ", end="")
        time.sleep(0.5)
        img_list = []
    ...
    for _ in range(EP_LEN_CHUNK):
        # return a chunk of actions, shape (fwd_pred_next_n, 7)
        action = model.step(obs, lang_annotation, env)
        # execute each action in the chunk
        for act_ind in range(action.shape[0]):
            obs, _, _, current_info = env.step(action[act_ind])
            if debug:
                img_copy = copy.deepcopy(obs["rgb_obs"]["rgb_static"])
                img_list.append(img_copy)
...
def parser_args():
    ...
    parser.add_argument(
        "--ablate",
        action="store_true",
        help="Save results for the same setting in different path/folder.",
    )
    parser.add_argument(
        "--act_chunk",
        action="store_true",
        help="if enabled, run the actual rollout on one subtask with action chunk, else single step with ensmbled action.",
    )
...
def main():
    ...
    if args.is_pt_config:
        ...
        from falcon.utils.config_utils import generate_calvin_ft_configs, deep_update

        ft_configs = generate_calvin_ft_configs(configs)
        deep_update(configs, ft_configs)

    # Get checkpoint path
    from falcon.utils.eval_utils import sort_ckpt
    ...
    from falcon.utils.config_utils import get_exp_name
