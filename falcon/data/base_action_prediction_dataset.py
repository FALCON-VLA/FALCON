...
from falcon.data.base_task_dataset import BaseTaskDataset, IGNORE_INDEX
...
class ActionPredictionBatchTransform:
    ...
    def convert_image_esm(
            ...
            images_vggt = deepcopy(images) # (ws+fwd, h, w, 3), np.ndarray
            image_tensors = self.image_fn(
                [Image.fromarray(each_image) for each_image in images], static=static
            )
            ...
            new_list_vggt = [] 
            for i in range(images_vggt.shape[0]):
                new_list_vggt.append(Image.fromarray(images_vggt[i, :, :, :].astype(np.uint8)))
            vggt_image_tensors, _ = load_and_preprocess_images_square_new(new_list_vggt, target_size=vggt_target_size) # [ws+fwd, 3, 224, 224]
            ...
            left_pad_index = self.window_size - image_mask[: self.window_size].sum()
            image_tensors[:left_pad_index] = image_tensors[left_pad_index]
            vggt_image_tensors[:left_pad_index] = vggt_image_tensors[left_pad_index]
            ...
            image_tensors = image_tensors[: self.window_size]
            vggt_image_tensors = vggt_image_tensors[: self.window_size]
            return image_tensors, image_chunk, image_chunk_mask, vggt_image_tensors
    ...
    def __call__(
        ...
        vggt_image_tensors = None
        if self.act_head_configs["type"] in {"FCDecoder_ESM"}:
            image_tensors, image_chunk, image_chunk_mask, vggt_image_tensors = self.convert_image_esm(
                images, episode_mask
            )
        else:
        ...
        return dict(
            ...
            # esm inputs
            vggt_image_tensors=vggt_image_tensors
        )
...
class ActionPredictionPaddedCollator:
    ...
    def __call__(
        ...
        if vggt_image_tensors[0] is not None:
            vggt_image_tensors = torch.stack(vggt_image_tensors)
        else:
            vggt_image_tensors = None
        gripper_image_tensors = (
...
class ActionPredictionDataset(BaseTaskDataset):
    ...
    def __init__(
        ...
        act_head_configs: Optional[Dict[str, Any]] = None,
        **kwargs,
    ):
        """
        Args:
            ...
            act_head_configs: the configs of action head
        """
        (
            self.model_name,
            self.mode,
        ...
        # debug
        # print("self.image_history init: ", self.image_history) # True
    ...
    def init_batch_transform(self):
        ...
        return ActionPredictionBatchTransform(
            ...
            min_action=self.min_action,
            max_action=self.max_action,
            act_head_configs=self.act_head_configs
        )
