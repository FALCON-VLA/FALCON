...
from falcon.utils.model_utils import update_tokenizer
from falcon.model.vlm_builder import build_vlm
from falcon.model.policy_head.action_tokenizer import ActionTokenizer
from falcon.data.vid_llava_constants import (
    IGNORE_INDEX,
    IMAGE_TOKEN_INDEX,
...
from falcon.model.text_encoder.clip_text_encoder import ClipTextFeatureEncoder
...
class BaseRoboVLM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if self.use_vision_resampler:
            from falcon.model.vision_encoder.vision_resampler import (
                PerceiverResampler,
            )
    ...
    def encode_state(self, state):
        ...
        state_embeddings = self.embed_state(state_embeddings)  # (b, l, h), h=2048 for kosmos
    ...
    def _init_heads(self):
        action_head = None
        if self.act_head_configs is not None:
            import falcon.model.policy_head as action_heads
            # init esm, load esm pretrained weights
            if self.act_head_configs["type"] in {"FCDecoder_ESM", "LSTMDecoder_ESM"}:
                ...
                esm_ckpt_path = self.act_head_configs.get('esm_ckpt_path')
                print('loading ESM pretrained weights from {}'.format(esm_ckpt_path))
                esm_net = VGGT_Camera_Replace_Depth().to(self.model.device).eval()
                checkpoint_esm = load_file(esm_ckpt_path)
                msg = esm_net.load_state_dict(checkpoint_esm, strict=False)
                print(f"ESM pretrained weights loading msg: {msg}")
                del checkpoint_esm
                _kwargs.update(
                    dict(  # hidden_size=self.hidden_size,
                        ...
                        vggt_net=esm_net,
                        esm_use_hand_rgb=self.act_head_configs.get("esm_use_hand_rgb", False),
                        camera_gt_pt=self.act_head_configs.get("camera_gt_pt", 0.0),
                        depth_gt_pt=self.act_head_configs.get("depth_gt_pt", 0.0),
                    )
                )
        ...
        fwd_decoder = None
        if self.fwd_head_configs is not None:
            import falcon.model.forward_head as fwd_heads
    ...
    def _trainable_params_setup(self):
        ...
        if self.act_head is not None:
            if self.train_setup_configs.get("train_act_head", True):
                self.act_head.requires_grad_(True)
            else:
                self.act_head.requires_grad_(False)

        # setup the training para of pcd_injector
        if self.act_head_configs.get("use_pcd_injector", False):
            if self.train_setup_configs.get("train_pcd_injector", True):
                self.act_head.pcd_encoder.requires_grad_(True)
                self.act_head.pcd_injector.requires_grad_(True)
            else:
                self.act_head.pcd_encoder.requires_grad_(False)
                self.act_head.pcd_injector.requires_grad_(False)

        # setup the training para of esm and spatial_injector
        if self.act_head_configs.get("use_spatial_injector", False):
            self.act_head.vggt_net.requires_grad_(False)
            if self.train_setup_configs.get("train_spatial_injector", True):
                self.act_head.spatial_injector.requires_grad_(True)
            else:
                self.act_head.spatial_injector.requires_grad_(False)
    ...
    def _forward_action_head_3D(
        self,
        action_tokens: torch.Tensor,
        action_labels: Tuple[torch.Tensor, torch.Tensor] = None,
        action_mask: torch.Tensor = None,
        static_pcd: torch.Tensor = None,
        gripper_pcd: torch.Tensor = None,
        **kwargs,
    ):
        # action_tokens = get_target_modal_tokens(output_hs, self._action_mask(output_hs))
        action = self.act_head(
            action_tokens, actions=action_labels, action_masks=action_mask, pcd_tensor=static_pcd, hand_pcd_tensor=gripper_pcd, **kwargs
        )
    ...
    def forward_continuous(
        ...
        gripper_pcd: torch.Tensor = None,
        val_mode: bool = False,
        rgb_vggt: torch.Tensor = None,
        static_depth_vggt: torch.Tensor = None,
        point_masks_vggt: torch.Tensor = None,
        static_cam_intr_mat_vggt: torch.Tensor = None,
        static_cam_ex_mat_vggt: torch.Tensor = None,
        mode="train",
        **kwargs,
    ):
        loss = {}
        ...
        if self.act_head_configs["type"] in {"FCDecoder_3D"}:
            if self.act_head_configs.get("use_hand_pcd", False):
                action_logits, action_loss = self._forward_action_head_3D(
                    action_tokens=action_hs, 
                    action_labels=action_labels, 
                    action_mask=action_mask,
                    static_pcd=static_pcd,
            ...
            action_logits, action_loss = self._forward_action_head(
                ...
                action_mask=action_mask,
                rgb_vggt=rgb_vggt,
                static_depth_vggt=static_depth_vggt,
                point_masks_vggt=point_masks_vggt,
                static_cam_intr_mat_vggt=static_cam_intr_mat_vggt,
                static_cam_ex_mat_vggt=static_cam_ex_mat_vggt,
            )                
