...
from falcon.train.train_utils import adjust_learning_rate
import falcon.model.backbone as RoboVLM_Backbone
...
class BaseTrainer(pl.LightningModule):
    ...
    def _init_policy(self):
        ...
        print("Model trainable params details: ")
        self._main_rank_print({k for k, v in model.named_parameters() if v.requires_grad})
        self._main_rank_print(f"{self.configs['robovlm_name']} Model Trainable Parameters: {total_params / 1000000:.2f}M")
        return model
    ...
    def _initialize(self):
        ...
        self.act_head_use_hand_rgb = self.configs["act_head"].get("use_hand_rgb", False)
    ...
    def from_checkpoint(cls, ckpt_path=None, ckpt_source="torch", configs=None):
        ...
        print("ckpt_source: {}".format(ckpt_source))
        ...
        if ckpt_source == "torch":
            ...
            del checkpoint
            from falcon.train.train_utils import convert_old_state_dict
    ...
    def configure_optimizers(self):
        ...
        self._main_rank_print(f"effective scheduler type: {scheduler_type}")
    ...
    def _process_batch(self, batch):
        ...
        if self.use_hand_rgb and batch.get("fwd_hand_rgb_chunck", None) is not None:
            fwd_hand_rgb_chunck = batch.get("fwd_hand_rgb_chunck").cuda()
        else:
        ...
        # prepare pcd data
        static_pcd = batch.get("static_pcd", None)
        if static_pcd is not None:
            static_pcd = static_pcd.cuda()
            static_pcd = static_pcd[:, :seq_len]

        gripper_pcd = batch.get("gripper_pcd", None)
        if gripper_pcd is not None:
            gripper_pcd = gripper_pcd.cuda()
            gripper_pcd = gripper_pcd[:, :seq_len]

        # prepare img, cam params and depth for esm
        rgb_vggt = None
        static_depth_vggt = None
        point_masks_vggt = None
        static_cam_intr_mat_vggt = None
        static_cam_ex_mat_vggt = None
        if self.configs["act_head"]["type"] in {"FCDecoder_ESM", "LSTMDecoder_ESM"}:
            rgb_vggt = batch["rgb_vggt"].cuda()
            rgb_vggt = rgb_vggt[:, :seq_len]

            static_depth_vggt = batch.get("static_depth_vggt", None)
            if static_depth_vggt is not None:
                static_depth_vggt = static_depth_vggt.cuda()
                static_depth_vggt = static_depth_vggt[:, :seq_len]
                point_masks_vggt = batch["point_masks_vggt"].cuda()
                point_masks_vggt = point_masks_vggt[:, :seq_len]

            static_cam_intr_mat_vggt = batch.get("static_cam_intr_mat_vggt", None)
            if static_cam_intr_mat_vggt is not None:
                static_cam_intr_mat_vggt = batch["static_cam_intr_mat_vggt"].cuda()
                static_cam_intr_mat_vggt = static_cam_intr_mat_vggt[:, :seq_len]
                static_cam_ex_mat_vggt = batch["static_cam_ex_mat_vggt"].cuda()
                static_cam_ex_mat_vggt = static_cam_ex_mat_vggt[:, :seq_len]
        ...
        return (
            ...
            static_depth_vggt,
            point_masks_vggt,
            static_cam_intr_mat_vggt,
            static_cam_ex_mat_vggt,
            data_source,
        )
