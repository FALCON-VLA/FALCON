...
from falcon.data.datamodule.gr_datamodule import GRDataModule
from falcon.data.data_utils import preprocess_image, policy_image_processor
from falcon.utils.setup_callback import SetupCallback
    ...
    if isinstance(trainer_config.get("logger"), list):
        ...
        for logger in trainer_config.get("logger"):
            if logger == "tensorboard":
                ...
                loggers.append(WandbLogger(save_dir=configs["log_dir"].as_posix(), name=exp_name, project="falcon-fc-oxe"))
            else:
...
def experiment(variant):
    ...
    _kwargs = {
        ...
        "datamodule": GRDataModule(
            ...
            image_fn_policy=functools.partial(
                preprocess_image,
                image_processor=policy_image_processor(image_size=224),
                model_type=variant["act_head"].get("type"),
            ),
            discrete=(
                False
            ...
            act_head_configs=variant.get("act_head", None),
            # vit_name=vit_name,
